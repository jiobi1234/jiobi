...
print(f"Testing Module 2: Route Optimization...")
print(f"Input Candidates: {len(mock_candidates['candidates'])}")
...
try:
    ...
    print("\n[SUCCESS] Optimized Plan Received:")
    print(f"Title: {result['title']}")
    
    for day in result['days']:
        print(f"\n[Day {day['day']}]")
        for item in day['schedule']:
            print(f"  {item['time']} - {item['place']} ({item['type']}): {item['description']}")
